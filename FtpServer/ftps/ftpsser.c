...
bool dataConnect( void )
{
  ...
  if( dataPassiveConn )
  {
    ...
    systime_t time = chTimeNow() + MS2ST( 500 );
...
{
  uint32_t deltaT = (uint32_t) ( chTimeNow() - timeBeginTrans );
...
void ftp_server_service( struct netconn *cn, struct netconn *dscn )
{
  ...
  CONSOLE_PRINT( "Client connected!\r\n" );
  ...
  netconn_set_recvtimeout( conn, FTP_TIME_OUT * 60 * 1000 );
  while( true )
  {
    err = readCommand();
    if( err == -4 ) // time out
  ...
  // netbuf_delete( inbuf );
  CONSOLE_PRINT( "Client disconnected\r\n" );
}
